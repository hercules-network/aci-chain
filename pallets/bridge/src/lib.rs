...
/// runtime module implementing Substrate side of PolkadaiBridge voucher exchange bridge
/// You can use mint to create vouchers backed by locked funds on Ethereum side
/// and transfer vouchers on substrate side freely
///
/// BUGS:
...
use crate::types::{
    ...
    VoucherBalance, TransferMessage, ValidatorsMessage,
};
use codec::{Encode};
use sp_core::H160;
...
use aci_primitives::{CurrencyId, Balance};
...
decl_event!(
    ...
    {
        RelayMessage(Hash),
        ApprovedRelayMessage(Hash, AccountId, H160, VoucherBalance),
        CancellationConfirmedMessage(Hash),
        MintedMessage(Hash),
        BurnedMessage(Hash, AccountId, H160, VoucherBalance),
...
decl_storage! {
    trait Store for Module<T: Config> as Bridge {
        ...
        pub DailyLimits get(fn daily_limits_by_account): map hasher(identity) T::AccountId => VoucherBalance;
...
decl_module! {
    pub struct Module<T: Config> for enum Call where origin: T::Origin {
        ...
        #[weight = 0]
        pub fn set_transfer(origin, to: H160, #[compact] amount: VoucherBalance)-> DispatchResult
        {
        ...
        #[weight = 0]
        pub fn multi_signed_mint(origin, message_id: T::Hash, from: H160, to: T::AccountId, #[compact] amount: VoucherBalance)-> DispatchResult {
...
impl<T: Config> Module<T> {
    ...
    fn lock_for_burn(message_id: T::Hash, account: T::AccountId, amount: VoucherBalance) -> DispatchResult {
        // lock currency
    ...
    fn check_daily_account_volume(account: T::AccountId, amount: VoucherBalance) -> DispatchResult {
    ...
    fn check_amount(amount: VoucherBalance) -> DispatchResult {
    ...
    fn check_pending_burn(amount: VoucherBalance) -> DispatchResult {
    ...
    fn check_pending_mint(amount: VoucherBalance) -> DispatchResult {
