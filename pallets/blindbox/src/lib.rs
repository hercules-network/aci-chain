...
decl_storage! {
    trait Store for Module<T: Config> as DeacBlindBox {
        /// Next CardGroup id
        ...
        /// CardGroup List
        pub CardGroupList get(fn get_card_group): map hasher(identity) u64 => DeacCard;
...
decl_module! {
    pub struct Module<T: Config> for enum Call where origin: T::Origin {
        ...
        pub fn blind_box_add_card_group(origin, blind_box_id: u64, collection_id: u64, item_id: u64, value: u64 ) -> DispatchResult {
            ...
            {
                pallet_deac::CollectionMode::DEAC(_) => T::DeacHandler::transfer_deac(collection_id, item_id, sender.clone(), locker)?,
                pallet_deac::CollectionMode::Fungible(_)  => T::DeacHandler::transfer_fungible(collection_id, item_id, value, sender.clone(), locker)?,
                pallet_deac::CollectionMode::ReFungible(_, _)  => T::DeacHandler::transfer_refungible(collection_id, item_id, value, sender.clone(), locker)?,
                _ => ()
            };
            ...
            CardGroupList::insert(group_id, deac_card.clone());
            NextCardGroupID::mutate(|id| *id += 1);

            let mut card_group = blind_box.clone().card_group;
            card_group.push(deac_card.group_id);
        ...
        pub fn blind_box_remove_card_group(origin, blind_box_id: u64, card_group_id: u64) -> DispatchResult {
            ...
            if card_value > 0 {
                match target_collection.mode {
                    pallet_deac::CollectionMode::DEAC(_) => T::DeacHandler::transfer_deac(card_group.collection_id, card_group.item_id, locker.clone(), sender.clone())?,
                    pallet_deac::CollectionMode::Fungible(_)  => T::DeacHandler::transfer_fungible(card_group.collection_id, card_group.item_id, card_value, locker.clone(), sender.clone())?,
                    pallet_deac::CollectionMode::ReFungible(_, _)  => T::DeacHandler::transfer_refungible(card_group.collection_id, card_group.item_id, card_value, locker.clone(), sender.clone())?,
                    _ => ()
                };
        ...
        pub fn buy_blind_box(origin, blind_box_id: u64, receive: Option<T::AccountId>) -> DispatchResult {
            ...
            if blind_box.price > 0 {
                <T as pallet_deac::Config>::MultiCurrency::transfer(currency_id, &sender, &blind_box.owner, blind_box.price.into())?;
            }

            let target_collection = pallet_deac::Module::<T>::collection(card_group.collection_id);
            match target_collection.mode
            {
                pallet_deac::CollectionMode::DEAC(_) => T::DeacHandler::transfer_deac(card_group.collection_id, card_group.item_id, locker, receive_user.clone())?,
                pallet_deac::CollectionMode::Fungible(_)  => T::DeacHandler::transfer_fungible(card_group.collection_id, card_group.item_id, 1, locker, receive_user.clone())?,
                pallet_deac::CollectionMode::ReFungible(_, _)  => T::DeacHandler::transfer_refungible(card_group.collection_id, card_group.item_id, 1, locker, receive_user.clone())?,
                _ => ()
            };
        ...
        pub fn cancel_blind_box(origin, blind_box_id: u64) -> DispatchResult {
            ...
            for card_group_id in blind_box.card_group.iter() {
                ...
                if card_group.remaind_value > 0 {
                    ...
                    match target_collection.mode {
                        pallet_deac::CollectionMode::DEAC(_) => T::DeacHandler::transfer_deac(card_group.collection_id, card_group.item_id, locker.clone(), sender.clone())?,
                        pallet_deac::CollectionMode::Fungible(_)  => T::DeacHandler::transfer_fungible(card_group.collection_id, card_group.item_id, card_group.remaind_value, locker.clone(), sender.clone())?,
                        pallet_deac::CollectionMode::ReFungible(_, _)  => T::DeacHandler::transfer_refungible(card_group.collection_id, card_group.item_id, card_group.remaind_value, locker.clone(), sender.clone())?,
                        _ => ()
                    };
                }
                CardGroupList::remove(card_group_id);
...
impl<T: Config> Module<T> {
    ...
    fn generate_random_number(seed: u32) -> u32 {
        let random_seed = T::Randomness::random(&(Self::deac_account_id(), seed).encode());
