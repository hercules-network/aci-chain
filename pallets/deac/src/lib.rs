...
pub struct CollectionType<AccountId> {
    ...
    pub voucher_prefix: Vec<u8>, // 16 include null escape char
...
#[derive(Encode, Decode, Default, Clone, PartialEq)]
#[cfg_attr(feature = "std", derive(Debug))]
pub struct DeacItemType<AccountId> {
    pub collection: u64,
    pub owner: AccountId,
    pub data: Vec<u8>,
    pub item_hash: H160,
...
decl_storage! {
    trait Store for Module<T: Config> as DEAC {
        ...
        /// Item collections
        pub DeacItemList get(fn deac_item_id): double_map hasher(blake2_128_concat) u64, hasher(blake2_128_concat) u64 => DeacItemType<T::AccountId>;
        ...
        /// Index list
        pub AddressVouchers get(fn address_vouchers): double_map hasher(blake2_128_concat) u64, hasher(blake2_128_concat) T::AccountId => Vec<u64>;
        ...
        /// Signature history
        pub SignatureList get(fn deac_signature_list): double_map hasher(blake2_128_concat) u64, hasher(blake2_128_concat) u64 => Vec<SignatureAuthentication<T::AccountId, T::BlockNumber, T::Name>>;
...
decl_module! {
    pub struct Module<T: Config> for enum Call where origin: T::Origin {
        ...
        // Create collection of DEAC with given parameters
        //
        ...
        pub fn create_collection(origin,
            ...
            let mut prefix = voucher_prefix.to_vec();
            prefix.push(0);
            ensure!(prefix.len() <= 16, "Voucher prefix can not be longer than 15 char");
        ...
        pub fn destroy_collection(origin, collection_id: u64) -> DispatchResult {
            ...
            // TODO Items remove
            <AddressVouchers<T>>::remove_prefix(collection_id);
        ...
        pub fn burn_item(origin, collection_id: u64, item_id: u64) -> DispatchResult {
            ...
            match target_collection.mode
            {
                CollectionMode::DEAC(_) => Self::burn_deac_item(collection_id, item_id)?,
        ...
        pub fn transfer(origin, recipient: T::AccountId, collection_id: u64, item_id: u64, value: u64) -> DispatchResult {
            ...
            match target_collection.mode
            {
                CollectionMode::DEAC(_) => Self::transfer_deac(collection_id, item_id, sender.clone(), recipient.clone())?,
        ...
        pub fn transfer_from(origin, from: T::AccountId, recipient: T::AccountId, collection_id: u64, item_id: u64, value: u64 ) -> DispatchResult {
            ...
            match target_collection.mode
            {
                CollectionMode::DEAC(_) => Self::transfer_deac(collection_id, item_id, from, recipient)?,
        ...
        pub fn create_sale_order(origin, collection_id: u64, item_id: u64, value: u64, price: u64 ) -> DispatchResult {
            ...
            if let CollectionMode::DEAC(_) = target_collection.mode {
                card_value = 1;
            };

            match target_collection.mode
            {
                CollectionMode::DEAC(_) => Self::transfer_deac(collection_id, item_id, sender.clone(), recipient)?,
        ...
        pub fn cancel_sale_order(origin, collection_id: u64, item_id: u64) -> DispatchResult {
            ...
            match target_collection.mode
            {
                CollectionMode::DEAC(_) => Self::transfer_deac(collection_id, item_id, locker, sender.clone())?,
        ...
        pub fn accept_sale_order(origin, collection_id: u64, item_id: u64) -> DispatchResult {
            ...
            <T as Config>::Currency::resolve_creating(&deac_owner, negative_imbalance);

            // Moves deac from locker account into the buyer's account
            match target_collection.mode
            {
                CollectionMode::DEAC(_) => Self::transfer_deac(collection_id, item_id, locker, sender.clone())?,
                CollectionMode::Fungible(_)  => Self::transfer_fungible(collection_id, item_id, target_sale_order.value, locker, sender.clone())?,
            ...
            // call event
            Self::deposit_event(RawEvent::ItemOrderSucceed(collection_id, item_id, sender, deac_owner.clone(), order_id, target_sale_order.value, price));
            Ok(())
        }
        ...
        pub fn accept_separable_sale_order(origin, order_id: u64, value: u64) -> DispatchResult {
            ...
            // call event
            Self::deposit_event(RawEvent::ItemSeparableOrderSucceed(order_id, collection_id, item_id, value, sender, deac_owner, price));
            Ok(())
        }
        ...
        pub fn finish_auction(origin, collection_id: u64, item_id: u64) -> DispatchResult {
            ...
            if let Some(winner) =  histories.last() {
                match target_collection.mode
                {
                    CollectionMode::DEAC(_) => Self::transfer_deac(collection_id, item_id, locker.clone(), winner.bidder.clone())?,
                ...
                match target_collection.mode
                {
                    CollectionMode::DEAC(_) => Self::transfer_deac(collection_id, item_id, locker.clone(), auction.owner.clone())?,
        ...
        pub fn cancel_auction(origin, collection_id: u64, item_id: u64) -> DispatchResult {
            ...
            // Moves deac from locker account into the owner's account
            match target_collection.mode {
                CollectionMode::DEAC(_) => Self::transfer_deac(collection_id, item_id, locker, sender.clone())?,
...
impl<T: Config> Module<T> {
    ...
    fn add_fungible_item(item: FungibleItemType<T::AccountId>) -> DispatchResult {
        ...
        Self::add_voucher_index(item.collection, current_index, owner.clone())?;
    ...
    fn add_deac_item(item: DeacItemType<T::AccountId>) -> DispatchResult {
        let current_index = <ItemListIndex>::get(item.collection)
            .checked_add(1)
            .expect("Item list index id error");
        ...
        Self::add_voucher_index(collection_id, current_index, item.owner.clone())?;

        <ItemListIndex>::insert(collection_id, current_index);
        <DeacItemList<T>>::insert(collection_id, current_index, item);
    ...
    fn burn_refungible_item(collection_id: u64, item_id: u64, owner: T::AccountId) -> DispatchResult {
        ...
        let item = collection
            ...
            .next()
            .unwrap();
        Self::remove_voucher_index(collection_id, item_id, owner.clone())?;
    ...
    fn burn_deac_item(collection_id: u64, item_id: u64) -> DispatchResult {
        ensure!(
            <DeacItemList<T>>::contains_key(collection_id, item_id),
            "Item does not exists"
        );
        let item = <DeacItemList<T>>::get(collection_id, item_id);
        Self::remove_voucher_index(collection_id, item_id, item.owner.clone())?;
        ...
        let new_balance = <Balance<T>>::get(collection_id, item.owner.clone())
            ...
            .unwrap();
        <Balance<T>>::insert(collection_id, item.owner.clone(), new_balance);
        <DeacItemList<T>>::remove(collection_id, item_id);
    ...
    fn add_voucher_index(collection_id: u64, item_index: u64, owner: T::AccountId) -> DispatchResult {
        let list_exists = <AddressVouchers<T>>::contains_key(collection_id, owner.clone());
        if list_exists {
            let mut list = <AddressVouchers<T>>::get(collection_id, owner.clone());
            ...
            <AddressVouchers<T>>::insert(collection_id, owner.clone(), list);
        } else {
            let mut itm = Vec::new();
            itm.push(item_index.clone());
            <AddressVouchers<T>>::insert(collection_id, owner, itm);
        }

        Ok(())
    }

    fn remove_voucher_index(
        collection_id: u64,
        item_index: u64,
        owner: T::AccountId,
    ) -> DispatchResult {
        ...
        if list_exists {
            ...
            if item_contains {
                ...
                <AddressVouchers<T>>::insert(collection_id, owner, list);
            }
        }
    ...
    fn move_voucher_index(
        collection_id: u64,
        item_index: u64,
        old_owner: T::AccountId,
        new_owner: T::AccountId,
    ) -> DispatchResult {
        Self::remove_voucher_index(collection_id, item_index, old_owner)?;
        Self::add_voucher_index(collection_id, item_index, new_owner)?;
    ...
    fn auction_lock_id(id: u64) -> [u8; 8] {
        ...
        lock_id[0..3].copy_from_slice(&*b"deac");
        lock_id
    }
}

impl<T: Config> DeacManager<T::AccountId, T::BlockNumber> for Module<T> {
    ...
    fn transfer_fungible(
        ...
        let new_owner_items = <AddressVouchers<T>>::get(collection_id, new_owner.clone());
        ...
        if amount == val64 && new_owner_account_id == 0 {
            ...
            Self::move_voucher_index(collection_id, item_id, owner.clone(), new_owner.clone())?;
        } else {
            let mut new_full_item = full_item.clone();
            ...
            if amount == val64 {
                Self::remove_voucher_index(collection_id, item_id, full_item.owner.clone())?;
    ...
    fn transfer_refungible(
        ...
        if amount == val64 && !new_owner_has_account {
            ...
            // update index collection
            Self::move_voucher_index(collection_id, item_id, old_owner.clone(), new_owner.clone())?;
        } else {
            let mut new_full_item = full_item.clone();
            new_full_item
                .owner
            ...
            if new_owner_has_account {
                ...
                Self::add_voucher_index(collection_id, item_id, new_owner.clone())?;
            }
    ...
    fn transfer_deac(
        ...
        let mut item = <DeacItemList<T>>::get(collection_id, item_id);
        ...
        <DeacItemList<T>>::insert(collection_id, item_id, item);
    ...
    fn is_item_owner(subject: T::AccountId, collection_id: u64, item_id: u64) -> bool {
        ...
        match target_collection.mode {
            CollectionMode::DEAC(_) => {
                <DeacItemList<T>>::get(collection_id, item_id).owner == subject
            }
            CollectionMode::Fungible(_) => {
